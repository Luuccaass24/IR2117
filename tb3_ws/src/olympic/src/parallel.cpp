...
int main(int argc, char * argv[]){
	...
	auto node = rclcpp::Node::make_shared("parallel");
	...
	while (!teleport->wait_for_service(std::chrono::seconds(1))) {
  	...
  	auto spawn = node->create_client<turtlesim::srv::Spawn>("/turtle1/spawn");
    while (!spawn->wait_for_service(std::chrono::seconds(1))) {
    		if (!rclcpp::ok()) {
      			RCLCPP_ERROR(node->get_logger(), "Interrupted while waiting for the service. Exiting.");
      			return 1;
    		}
	...
	double radius = node->get_parameter("radius").get_parameter_value().get<double>();
        ...
        std::vector<std::string> names = {"turtle1","turtle2","turtle3","turtle4","turtle5"}
        ...
        for (int l =0;l<5;l++){
            ...
            request->y = pos[l][1];
            request->theta = 0.0;
            request->name = names[l];
            auto spawn_future = client->async_send_request(spawn_request);
            ...
            set_pen_request->turtle_name = names[l];
